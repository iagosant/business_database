...
# To separate a specific type of category, pair this with an empty array before first loop
  ...
  if ["BANK", "BANKING", "BANKS", "FEDERAL SAVINGS",
    ...
    "COMMERCIAL BANK", "BANK (COMMERCIAL)"].include? category
    @finance << row
  elsif ["ARMORED CAR SERVICE", "ARMORED"].include? category
...
@finance.each do |b_info|
  ...
  "BANK BUILDERS INTERNATIONAL INC", "TD BANK"].exclude? b_info[3]
    @local_bank_name << b_info[3]
    @local_bank_data << Hash["name" => b_info[3], "address" => b_info[4], "contact person" => b_info[2], "zipcode" => b_info[4].split(", ")[1].split(" ")[1]]
  end
end
